...
在 [新Idea探讨_20260613_等变GNN与自进化架构](../../24.md) 和 [SGD-Harness物理守恒哨兵与等变自进化闭环](../../25.md) 之后，推荐把 SGD-Net 的增强定义进一步写成：
...
在 [SGD-Net与JEPA世界模型融合及JSBO桥梁算子](../../18.md)、[SGD-Net混合求解训练体系与世界模型研究路线](../../27.md) 和 独立研究材料（本包不附） 的最新梳理之后，还需要补充一个系统级边界：`13` 号文档可以作为**算法内核与算子 SSOT**，但不能单独替代 `11/16/30/31` 的硬件系统文档、`03-06` 的实现计划、`18` 的 JSBO/LeJEPA 理论文档或 `SGD-Trace与Benchmark规范/` 的证据合同。对 `00`～`31` 的全部场景，应采用“内生核心 + 扩展模块 + 外部后端/peer IP”的分层支持方式。
...
[← 上一页](../../13.md) · [全书目录](../../../SUMMARY.md) · [下一页 →](section-02.md)
