...
2026-09-13 接入合同见[记忆—推理巩固与并行生成](../../../guide/memory-reasoning-parallel-generation.md)。连续生成已有实例；离散 token 路径仍需损坏过程、损失与采样定义。同轮读取冻结快照，生成时钟不得推进 D04 物理时间；局部更新必须处理依赖与缓存失效。
